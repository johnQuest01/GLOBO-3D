...
const estadosDe = (pais: string): LinhaComCoordenada[] => {
  const achado = rotulos.find(
    (p) =>
      p.name.toLowerCase() === pais.toLowerCase() ||
      p.key.toLowerCase() === pais.toLowerCase(),
  );
  try {
...
teste("Odessa: o pais desempata (Ucrania x Estados Unidos)", () => {
  ...
  const us = ondeFica(
    { country: "Estados Unidos", city: "Odessa" },
    semEstados,
  );
...
teste("Portland: o ESTADO desempata dentro do mesmo pais", () => {
  ...
  console.log(
    `      (${km(oregon!, maine!)} km entre elas, e cada uma no seu estado)`,
  );
});
...
teste("Springfield esta em cinco estados e cada um acha o seu", () => {
  ...
  for (const uf of [
    ...
    const r = ondeFica(
      { country: "Estados Unidos", state: uf, city: "Springfield" },
      eua,
    );
...
console.log("\n--- acento, caixa e rotulo de pais ---");

teste('"SAO PAULO" sem acento e em caixa alta acha Sao Paulo', () => {
  // Quatro contas reais tinham ficado no CENTRO do Brasil por causa disto: o
  // nome nao casava, caia para o pais, e ninguem via erro nenhum.
  const r = ondeFica(
    { country: "Brasil", state: "SP", city: "SAO PAULO" },
    brasil,
  );
  if (r?.precisao !== "cidade")
    throw new Error(`resolveu por ${r?.precisao ?? "nada"}`);
  perto(r, { lat: -23.55, lon: -46.63 }, 30);
});

teste('Hong Kong com pais "China" ainda acha Hong Kong', () => {
  /*
   * A lista traz o pais como "Hong Kong S.A.R."; quem escreve "China" nao casa
   * com ela. Como o nome e' unico no mundo, nao ha' o que desempatar — e uma
   * conta real tinha ido parar a 1.500 km dali, no meio da China continental.
   */
  const hk = { paises, cidades, estados: estadosDe("China") };
  const r = ondeFica({ country: "China", city: "Hong Kong" }, hk);
  if (r?.precisao !== "cidade")
    throw new Error(`resolveu por ${r?.precisao ?? "nada"}`);
  perto(r, { lat: 22.31, lon: 114.18 }, 60);
});

teste("mas com nome REPETIDO o pais continua mandando", () => {
  // Odessa existe na Ucrania e nos Estados Unidos. Pedir Odessa "no Brasil" nao
  // pode devolver nenhuma das duas: ali o pais e' a unica coisa que separa.
  const br = ondeFica({ country: "Brasil", city: "Odessa" }, brasil);
  if (br?.precisao === "cidade")
    throw new Error("pegou uma Odessa de outro pais");
});
...
teste("lugar inventado nao resolve (o formulario recusa salvar)", () => {
  const r = ondeFica(
    { country: "Narnia", state: "Ninguendia", city: "Lugarnenhum" },
    brasil,
  );
  if (r !== null)
    throw new Error(`situou o que nao existe: ${JSON.stringify(r)}`);
});
...
teste("recusa NaN, texto, nulo e fora do planeta", () => {
  ...
  for (const [la, lo] of ruins) {
    if (coordenadaValida(la, lo))
      throw new Error(`aceitou ${String(la)}, ${String(lo)}`);
  }
});
